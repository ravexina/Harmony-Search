...
class HarmonySearch:
    ...
    def init(self):
        ...
        self.HM = np.full((self.memory_size, self.instruments), None)

        for i in range(self.memory_size):
            available_cities = [i for i in range(self.lower, self.upper)]
            rnd.shuffle(available_cities)
            for n in range(self.instruments - 1):
                self.HM[i][n] = available_cities.pop()

        for i in range(self.memory_size):
            self.HM[i][self.instruments - 1] = self.HM[i][0]
    ...
    def improvise(self):
        ...
        for i in range(self.instruments):
            ...
            # Last loop, put first node at last
            if i == self.instruments - 1:
                new_harmony[i] = new_harmony[0]
                break
            ...
            if np.random.uniform(0, 1) < self.HMCR and not f:
                # From Harmony Memory
                while True:
                    choice = np.random.choice(self.HM[:, i])
                    if choice not in new_harmony:
                        new_harmony[i] = choice
                        break
            else:
                # Random
                while True:
                    rand = np.random.randint(self.lower, self.upper)
                    if rand not in new_harmony:
                        new_harmony[i] = rand
                        break
